...
const StatusBadge = ({ status }: { status: ProposedProjectStatus }) => {
  ...
  switch (status) {
    ...
    case 'APPROVED_BY_HEAD':
      displayLabel = 'Đã được phê duyệt';
      color = 'success';
      icon = <CheckCircleIcon fontSize="small" />;
      break;
    default:
                        ...
                        {outline.fileId && (
                          <Box>
                            ...
                            >
                              ...
                              <Typography variant="body2">
                                {outline.file?.originalName || 'Tệp đính kèm'}
                            ...
                            {/* PDF Viewer for inline display */}
                            {
                              <Box sx={{ mt: 2 }}>
                                <Button
                                  ...
                                  disabled={!outline.fileId}
                                  size="small"
                                >
                                  Tải xuống{' '}
                                  {outline.file?.originalName || 'Đề cương'}
                                </Button>
                              </Box>
                            }
                          </Box>
